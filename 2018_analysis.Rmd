...
pkgs <- c("WGCNA","phyloseq", "microbiome", "vegan",'egg','adespatial', 'corrplot','RColorBrewer','tidymodels','corrr','GGally','patchwork',"circlize","multcomp",
          'tidytext','hrbrthemes','embed', 'pvclust','skimr', 'RColorBrewer', 'grid',"pls","tidyverse") # Load packages
...
OTUdf <- OTUdf %>% dplyr::select(-MH063SR, -MH028SC) # replicate samples
```
...
# remove extra text
TAXdf <- TAXdf %>% rownames_to_column("OTU") %>%
...
# new tax table does not have the smae OTUs. Subsetting OTU table
...
write.csv(df.env, "C:/Users/rrocha/Documents/R/phd/2018/output/data/all/env/df_env.csv")
# Summary 
...
write.csv(meta_summary.site, "C:/Users/rrocha/Documents/R/phd/2018/output/data/all/env/meta_summary_site.csv")
```
...
stats.env <- meta_long %>% 
  ...
  select(env_par, tidy) %>% 
  unnest(tidy)
write.csv(stats.env, "C:/Users/rrocha/Documents/R/phd/2018/output/data/all/env/anova_env_summary.csv")
...
stats.env.Tukey.BF <- meta_long %>% 
  ...
  unnest(tidy) 
write.csv(stats.env.Tukey.BF, "C:/Users/rrocha/Documents/R/phd/2018/output/data/all/env/anova_env_summary_tukey_BF.csv")

# only sig
env.sig.tukey.BF <- stats.env.Tukey.BF %>% filter(adj.p.value < 0.001)                    write.csv(env.sig.tukey.BF, "C:/Users/rrocha/Documents/R/phd/2018/output/data/all/env/anova_env_summary_tukey_sig_BF.csv")
...
pca.plot <- juice(pca_prep) %>% # juice() to return the results of a recipe
        mutate(Layer = fct_relevel(Layer, c("S", "P"))) %>%
  ggplot(aes(PC1,PC2)) +
  geom_point(aes(color = Site_Layer, shape = Layer), alpha = 0.7, size =5) + 
...
# save as csv
write.csv(perm.tb, "C:/Users/rrocha/Documents/R/phd/2018/output/data/all/permanova_env.csv")
```
...
- Notes: 
 ...
 + Simpson's (D):less sensitive to the difference in taxa richness than Shannon's index - high values mean less diversity (range: 0-1)
...
for (i in seq_along(pseq.rar.list)) {
  ...
  df <- sample_data(pseq.rar.list[[i]]) %>% 
    ...
    summarise(mean= mean(value), sd= sd(value), max = max(value),min = min(value))
  write.csv(df, file = paste0("C:/Users/rrocha/Documents/R/phd/2018/output/data/all/alpha/alpha_summary.Site_Layer_", names(pseq.rar.list[i]), ".csv"))
  ...
  df <- sample_data(pseq.rar.list[[i]]) %>% 
    ...
    select(env_par, tidy) %>% 
    unnest(tidy)
  write.csv(df, file = paste0("C:/Users/rrocha/Documents/R/phd/2018/output/data/all/alpha/alpha_anova_", names(pseq.rar.list[i]), ".csv"))
  ...
  df <- sample_data(pseq.rar.list[[i]]) %>% 
    ...
    select(env_par, tidy) %>% 
    unnest(tidy)
  write.csv(df, file = paste0("C:/Users/rrocha/Documents/R/phd/2018/output/data/all/alpha/alpha_Tuk_", names(pseq.rar.list[i]), ".csv"))
 
   boxplot_alpha <- sample_data(pseq.rar.list[[i]]) %>% 
    unclass() %>% data.frame() %>%
    left_join(adiv, by = "Sample_ID") %>%
    ...
    scale_x_discrete(limits = c("ocean", 'C', 'B', 'A','GR')) +  
    scale_color_manual(values = cols_layer) +
    ggtitle(paste0("Alpha Diveristy - Surface vs Intermediate Layers - ",names(pseq.rar.list[i]))) +
    theme_Publication_3() + 
...
# run the correlation 
  cor.alpha_env <- meta %>% left_join(adiv) %>% select(Temperature:Pielou, -pH, -Turbidity, -DO, -DO_perc,-se.chao1) %>%  
    ...
    select(term:NH4)
# save
  write.csv(cor.alpha_env, file = paste0("C:/Users/rrocha/Documents/R/phd/2018/output/data/all/alpha/corr_alpha_env_", names(pseq.rar.list[i]), ".csv"))
...
for (i in seq_along(pseq.list)) {
  ...
  for (j in seq(unique(Rank))) {
    ...
    write.csv(summary_rel[[j]], paste0('C:/Users/rrocha/Documents/R/phd/2018/output/data/all/composition/summary_tables/all/rel_summary_all_',tax[[j]],'_',names(pseq.list[i]),".csv"))
    name <- paste(names(pseq.list[i]))
...
for (i in seq_along(pseq.list)) {
  ...
  for (j in seq(unique(Rank))) {
    ...
    write.csv(table[[j]], paste0('C:/Users/rrocha/Documents/R/phd/2018/output/data/all/composition/summary_tables/group/rel_summary_',Group,'_',tax[[j]],"_",names(pseq.list[i]),".csv"))
    name <- paste(names(pseq.list[i]))
...
for (i in seq_along(pseq.list)) {
  ...
  for (j in seq(unique(Rank))) {
    ...
    summary_rel_subtax[[j]] <- summarize_by_subtaxa(pseq.list[[i]], str_c(tax[[j]]), 'Family','Site_Layer') %>%
      ...
      slice_head(n = 5)
    write.csv(summary_rel_subtax[[j]], paste0('C:/Users/rrocha/Documents/R/phd/2018/output/data/all/composition/summary_tables/summary_subtax/rel_summary_Site_Layer_',tax[[j]],"_",names(pseq.list[i]),".csv"))
    name <- paste(names(pseq.list[i]))
...
for (i in seq_along(pseq.clr.list)) {
  ...
  for (i in seq_along(df)) {
    write.csv(df[[i]], paste0('C:/Users/rrocha/Documents/R/phd/2018/output/data/all/beta_div/beta_div_habitat_layer_',names(df[i]),names(pseq.clr.list[i]),'.csv'))
