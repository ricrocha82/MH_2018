...
# k=softConnectivity(datE=otu_WGCNA2,power=20)
...
#R^2 of ~0.8, this suggests we meet the assumption of scale-free topol.

# One-step network construction and module detection
# block-wise network construction and module detection
# power of 20 chosen based on powerEstimate from 'pst'
...
# to plot all plots for the env.par
#par(mfrow = c(2,3))  
...
# EUK
#---------------
# Chl = yellow (but in this module there is only one taxa, skip this analysis)
#env.par <- "Chlorophyll"
#module <- "yellow"
#thr_con <- 0.5 # eukaryotes - Chl/yellow

# Chl = grey (grey module is where all genes that doesn't fall into other modules)
env.par <- "Chlorophyll"
module <- "grey"
thr_con <- 0.2 # eukaryotes - Chl/grey

# NH4 = turquoise (no correlation in pls, NO GOOD!!!)
#env.par <- "NH4"
#module <- "turquoise"
#thr_con <- 0.2 # eukaryotes - NH4/turquoise

# nox = blue
env.par <- "Nox"
module <- "blue"
thr_con <- 0.2 # eukaryotes - Nox/blue
...
n_top <- 10 # to plot the names (TOP VIP in)
...
hive <- hive  %>%
  mutate(names = paste0(OTU,"(",Class,", ",Family,")"))

# get the top OTUs
hive %>% slice_max(n=10, connectivity) %>% select(OTU, Order, Family, connectivity)
hive %>% slice_max(n=10, .data[[GS.envpar]])
hive %>% slice_max(n=10, VIP)
...
# taxa cmost correlated to env trait
hive %>% 
...
p <- hive %>%
  ...
  geom_point(aes(size = VIP, colour = .data[[taxon]], alpha = 0.5)) +
